export default function Controls({
	...
	function addMinutes() {
		if (Number(minutes.textContent) < 55) {
			minutes.textContent = String(Number(minutes.textContent) + 5).padStart(
				2,
				'0'
			)
		} else {
	...
	function counter() {
		time = setTimeout(() => {
			if (Number(second.textContent) == 0 && Number(minutes.textContent) > 0) {
				...
				second.textContent = String(Number(second.textContent) - 1).padStart(
					2,
					'0'
				)
				minutes.textContent = String(Number(minutes.textContent) - 1).padStart(
					2,
					'0'
				)
			} else {
				if (
					Number(minutes.textContent) == 0 &&
					Number(second.textContent) == 0
				) {
					reset()
					return
				} else {
